...
func moderateHandler(c echo.Context) error {
	...
	// ライブコメント一覧取得（このlivestreamのみ、一度だけ取得）
	...
	for _, livecomment := range livecomments {
		for _, ngword := range ngwords {
			if strings.Contains(livecomment.Comment, ngword.Word) {
				...
				break // このコメントは削除対象なので、他のNGワードはチェック不要
			}
		}
	}
	...
	if len(deleteIDs) > 0 {
		...
		if err != nil {
			return echo.NewHTTPError(http.StatusInternalServerError, "failed to construct IN query for delete: "+err.Error())
		}
